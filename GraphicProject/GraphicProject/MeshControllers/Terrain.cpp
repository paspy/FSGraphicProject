...
void Terrain::Init(ID3D11Device* device, ID3D11DeviceContext* _d3dDevice, const InitInfo& initInfo) {
	mInfo = initInfo;
	...
	mLayerMapArraySRV = D3DUtils::CreateTexture2DArraySRV(device, _d3dDevice, layerFilenames);

	HR(CreateDDSTextureFromFile(device, mInfo.BlendMapFilename.c_str(), NULL, &mBlendMapSRV));
}

void Terrain::Render(ID3D11DeviceContext* _d3dDevice, const Camera& cam, DirectionalLight lights[3]) {
	_d3dDevice->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_4_CONTROL_POINT_PATCHLIST);
	_d3dDevice->IASetInputLayout(InputLayouts::Terrain);

	UINT stride = sizeof(Vertex::Terrain);
	UINT offset = 0;
	_d3dDevice->IASetVertexBuffers(0, 1, &mQuadPatchVB, &stride, &offset);
	_d3dDevice->IASetIndexBuffer(mQuadPatchIB, DXGI_FORMAT_R16_UINT, 0);
	...
	D3DUtils::ExtractFrustumPlanes(worldPlanes, viewProj);
	...
	for (UINT i = 0; i < techDesc.Passes; ++i) {
		...
		pass->Apply(0, _d3dDevice);

		_d3dDevice->DrawIndexed(mNumPatchQuadFaces * 4, 0, 0);
	}

	// FX sets tessellation stages, but it does not disable them.  So do that here
	// to turn off tessellation.
	_d3dDevice->HSSetShader(0, 0, 0);
	_d3dDevice->DSSetShader(0, 0, 0);
}

void Terrain::LoadHeightmap() {
	// A height for each vertex
	vector<unsigned char> in(mInfo.HeightmapWidth * mInfo.HeightmapHeight);
...
void Terrain::BuildQuadPatchVB(ID3D11Device* device) {
	vector<VertexT> patchVertices(mNumPatchVertRows*mNumPatchVertCols);
	...
	vbd.ByteWidth = sizeof(VertexT) * patchVertices.size();
...
void Terrain::BuildQuadPatchIB(ID3D11Device* device) {
	vector<USHORT> indices(mNumPatchQuadFaces * 4); // 4 indices per quad face
